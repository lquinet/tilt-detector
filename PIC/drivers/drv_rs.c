...
/* 2005/03/15 [XM]  Fixed a bug in "RS_RX_INT" and set RX_ByteNumber  */
/*                  to 0 in RS_RCV_Register.                          */
/* 2005/05/04 [XM]  Added the "pattern" field of the RS_RCV_Register  */
/*                  function to be awaked on a special character.     */
/*                                                                    */
/**********************************************************************/
...
{  
  ...
  SPBRG =  25; // for 19200  [8MHZ only] -0.16% error
  TXSTAbits.BRGH   = 1;
  BAUDCONbits.BRG16   = 0;
  ...
  PIR1bits.TXIF    = 0;
  PIR1bits.RCIF    = 0;
  PIE1bits.RCIE    = 0;//La reception n'est pas utilisée.
  PIE1bits.TXIE    = 0;  
...
void RS_RX_INT(void)
{
  ...
  {
    RS_list_head_rcv->data[RX_ByteNumber] = RCREG;
    if (RS_list_head_rcv->pattern == RCREG)
    {
      SetEvent(RS_list_head_rcv->CallerID, RS_QUEUE_FULL);
      RX_ByteNumber = -1;
    }
